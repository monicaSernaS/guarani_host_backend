...
exports.filterBookingsByType = exports.filterBookingsByStatus = exports.filterBookingsByDateRange = exports.deleteBookingByAdmin = exports.updateBookingPaymentStatus = exports.updateBookingByAdmin = exports.getAllBookingsForAdmin = void 0;
...
const deleteBookingByAdmin = async (req, res) => {
    try {
        const booking = await BookingModel_1.Booking.findByIdAndDelete(req.params.id);
        if (!booking) {
            res.status(404).json({ message: "🚫 Booking not found" });
            return;
        }
...
const filterBookingsByStatus = async (req, res) => {
    try {
        ...
        if (status)
            (0, validateBooking_1.validatePaymentStatus)(status);
        if (paymentStatus)
        ...
        const bookings = await BookingModel_1.Booking.find(query)
            .populate("user", "-password")
            .populate("property")
            .populate("tourPackage");
        res.status(200).json({
...
const filterBookingsByType = async (req, res) => {
    try {
        ...
        const bookings = await BookingModel_1.Booking.find(filter)
            .populate("user", "-password")
            .populate("property")
            .populate("tourPackage");
        res.status(200).json({

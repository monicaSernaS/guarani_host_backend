...
exports.updateHostTourStatus = exports.deleteHostTour = exports.updateHostTour = exports.getHostTourById = exports.getHostTours = exports.createHostTour = void 0;
 ...
 */
const createHostTour = async (req, res) => {
    try {
        ...
        if (status && !Object.values(enums_1.TourPackageStatus).includes(status)) {
        ...
        const newTour = new TourPackageModel_1.TourPackage({
            title: title.trim(),
...
exports.createHostTour = createHostTour;
/**
 * @desc    Host fetches all their tour packages
 * @route   GET /api/host/tours
 * @access  Private (host only)
 */
const getHostTours = async (req, res) => {
    try {
        const hostId = req.user?._id;
        ...
        const tours = await TourPackageModel_1.TourPackage.find({ host: hostId }).sort({ createdAt: -1 });
...
const getHostTourById = async (req, res) => {
    try {
        ...
        const tour = await TourPackageModel_1.TourPackage.findOne({
            _id: req.params.id,
            host: hostId
        });
        if (!tour) {
...
exports.getHostTourById = getHostTourById;
/**
 * @desc    Host updates one of their tour packages
 * @route   PATCH /api/host/tours/:id
 * @access  Private (host only)
 */
const updateHostTour = async (req, res) => {
    try {
        const hostId = req.user?._id;
        if (!hostId) {
            res.status(401).json({ message: "🚫 Unauthorized" });
        ...
        const { title, description, price, duration, maxCapacity, location, amenities, status, removedImages } = req.body;
...
exports.updateHostTourStatus = updateHostTourStatus;
